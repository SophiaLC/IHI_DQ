early_lag <- function(data) {
  ...
  Time_Diff=Early_Lag%>%
    mutate(lag_Arrival_Visit=as.numeric(difftime(Arrived,Visit,units="hours")),
  ...
  Lag_Summary=Time_Diff %>%
    ...
    summarise(Arrival_Visit=round(mean(lag_Arrival_Visit,na.rm=TRUE),2),
              Arrival_Message=round(mean(lag_Arrival_Message,na.rm=TRUE),2),
              Message_Record=round(mean(lag_Message_Record,na.rm=TRUE),2),
              Record_Visit=round(mean(lag_Record_Visit,na.rm=TRUE),2))
  )
